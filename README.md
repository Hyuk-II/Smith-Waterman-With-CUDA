...
### 🔬 Nsight Compute 프로파일링: 전역 메모리(`sw_gpu`) vs 공유 메모리 타일링(`sw_gpu_tiled`)

위 GCUPS(지표 1)가 *왜* 그렇게 나오는지를 하드웨어 단위로 분해하기 위해, Nsight Compute의 **Baseline 비교** 기능으로 두 커널의 중앙 반대각선(대표) 커널을 프로파일링했습니다.
모든 수치는 `Baseline = sw_gpu(전역)` 대비 `Current = sw_gpu_tiled(공유)` 의 증감입니다. (측정: RTX 4080 Super / Windows, 입력 ~3,584 AA)

#### 지표 2. Memory Bandwidth — 병목을 DRAM에서 온칩(L2/Shared)으로 이동

![GPU Speed Of Light](99_archive/profiling/tiled_vs_global/gpu_speed_of_light.png)
![Memory Workload Analysis](99_archive/profiling/tiled_vs_global/memory_workload_analysis.png)

| 지표 | `sw_gpu` (전역) | `sw_gpu_tiled` (공유) | 해석 |
|---|---:|---:|---|
| **DRAM Throughput** | 7.51 % | **1.01 %** | 전역 메모리 압력 약 **7.4배 감소** |
| Memory 대역폭 | 53.35 GB/s | **7.24 GB/s** (−86 %) | DRAM 실제 전송량 급감 |
| **L2 Hit Rate** | 42.02 % | **96.31 %** | 데이터 재사용·지역성 대폭 향상 |
| Shared Memory 사용 | 없음 | 4.42 KB/block | **타일링 실제 작동 증명** |

- 공유 메모리 타일링으로 연산 데이터가 느린 **DRAM 대신 L2/Shared**에서 처리됨이 정량적으로 증명됩니다 (DRAM −86 %, L2 적중 42 % → 96 %).
- Mem Busy·Max Bandwidth가 *오른* 것은 악화가 아니라 **병목이 DRAM → 온칩 메모리 파이프로 이동**한 결과입니다. 다만 절대 사용률이 모두 20 % 미만이라, 이 커널은 더 이상 메모리 대역폭에 묶이지 않습니다 → 한계는 **점유율(지표 3)** 로 넘어갑니다.

> ⚠️ **공유 메모리 뱅크 컨플릭트 (개선 여지):** Nsight Compute는 타일 커널의 `H_s` 접근에서 평균 **store 15.2-way / load 9.3-way** 뱅크 컨플릭트를 보고합니다. 원인은 `H_s[TILE_SIZE+1][TILE_SIZE+1]`의 행 stride가 **33(홀수)** 이기 때문입니다. 타일 내부 연산은 **반대각선 접근**(`i = 상수 − tx, j = 상수 + tx`)이라 오프셋이 `상수 − 32·tx` 가 되어 **한 대각선의 모든 스레드가 같은 뱅크**에 몰립니다. 일반 행/열 접근에서는 stride 홀수가 충돌을 피하지만, 반대각선 접근에서는 유효 stride가 `1 − S`라서 **`S`(둘째 차원)를 짝수**로 만들어야 충돌이 사라집니다. 즉 `H_s[TILE_SIZE+1][TILE_SIZE+2]`(stride 33 → 34)로 패딩 칸을 하나 더 두면 됩니다. (경계(halo)용 `+1`과 별개로, 메모리 최적화용 `+1`을 둘째 차원에만 추가)

#### 지표 3. Hardware Occupancy — 웨이브프론트의 구조적 유휴

![Occupancy](99_archive/profiling/tiled_vs_global/occupancy.png)
![Launch Statistics](99_archive/profiling/tiled_vs_global/launch_statistics.png)

| 지표 | `sw_gpu` (전역) | `sw_gpu_tiled` (공유) | 해석 |
|---|---:|---:|---|
| Theoretical Occupancy | 100 % | **37.5 %** | 공유 메모리가 블록 수를 제한(Block Limit Shared Mem) |
| Achieved Occupancy | 13.98 % | **2.82 %** | 둘 다 한 자릿수 → SM 미충전 |
| Active Warps / SM | 6.71 | 1.35 | (하드웨어 최대 48) |
| Waves per SM | 0.03 | 0.08 | **< 1 → GPU를 한 번도 못 채움** |

- 타일 커널의 이론 점유율은 `H_s` 공유 메모리 사용량 때문에 **37.5 %로 제한**됩니다 (*"limited by the required amount of shared memory"*).
- 실측 점유율이 한 자릿수로 떨어지는 본질적 원인은 **Waves per SM ≪ 1** — 가장 큰 중앙 대각선조차 블록 수가 80개 SM을 채우지 못하는 **웨이브프론트의 구조적 유휴**입니다 (Launch Statistics의 *"grid is too small to fill the available resources"*).
- 즉 타일링은 메모리(지표 2)는 살렸지만 점유율은 낮습니다. 그럼에도 전체가 **1.38배 빠른** 이유는 **커널 런치 32배 감소 + DRAM 트래픽 86 % 절감**이 점유율 손실을 압도하기 때문입니다 — 점유율은 목적이 아니라 수단임을 보여줍니다. (추가 최적화 여지: Shared Memory 뱅크 컨플릭트 제거, grid 충전율 향상)
