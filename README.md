...
> **GPU 공유 메모리 타일링 및 반대각선 파도타기(Wave-front) 스케줄링을 활용한 초고속 단백질/유전체 서열 정렬 가속기**

## 💡 1. Background & Objective

현대의 유전체 데이터는 기하급수적으로 폭증하고 있으나, 기존의 동적 계획법(DP) 기반 스미스-워터맨(Smith-Waterman) 알고리즘은 데이터 의존성으로 인해 병목이 존재합니다.

본 프로젝트는 알고리즘의 **반대각선 독립성**을 기반으로, 이를 GPU의 수천 개 코어에 매핑하여 연산 스텝을 $O(N^2)$에서 $O(N)$으로 개선합니다. 특히, 느린 전역 메모리 의존도를 낮추기 위해 **Shared Memory 기반의 Tiling** 기법을 적용하여 높은 연산 처리량을 달성하는 것을 목표로 합니다.
